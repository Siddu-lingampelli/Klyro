/**
 * Klyro TUI GÇö opencode-clean GÇö no clumsy words, correct wrap, markdown, scroll
 * Header 3 rows, guide Göé at col2, GùÅ Klyro accent, prose wrapped at word boundaries
 */
  ...
  return (
    <Box flexDirection="column" marginBottom={1}>
      <Box justifyContent="space-between">
        ...
        {showLinks ? <Text color={tokens.colors.dim as string}>Göé  /help   /config   /clear   /exit</Text> : null}
      </Box>
...
// Simple markdown: **bold** GåÆ bold, keep lists/tables, wrap at word boundaries
function MarkdownText({ text, dim, width }: { text: string; dim?: boolean; width?: number }) {
  ...
  // Render as single line with bold segments GÇö Ink will wrap the parent Box
...
export function App(props: AppProps): React.JSX.Element {
  ...
  const baseHints = status.status === 'running' ? 'ctrl+c to stop  -+  enter to queue  -+  ctrl+o expand' : transcript.length === 0 ? 'shift+tab to cycle  -+  GåæGåô for history  -+  / for commands' : 'enter to send  -+  shift+enter newline  -+  @ to attach';
    ...
    <Box flexDirection="column" width={width} height={isFullscreen ? height - 1 : undefined}>
      ...
      <Box flexDirection="row" flexGrow={isFullscreen ? 1 : 0} overflow={isFullscreen ? 'hidden' : undefined}>
        <Box flexDirection="column" flexGrow={1} overflow={isFullscreen ? 'hidden' : undefined} paddingX={0}>
          {grouped.length === 0 ? (
            <Text color={tokens.colors.dim as string}>Message KlyroGÇª</Text>
            ...
            if ((item as Group).verb) {
              ...
              const right = gr.status === 'running' ? `${(elapsed / 1000).toFixed(1)}s` : gr.status === 'error' ? 'G£ù' : `${gr.totalMs}ms`;
              const marker = isExpanded ? 'Gû+' : 'G£ô';
            ...
            if (it.kind === 'text') {
              // prose GÇö render markdown, not raw **, with proper wrap and guide
              return (
            ...
            if (it.kind === 'error') return <Box key={it.id} paddingLeft={2} marginBottom={1}><Text color={tokens.colors.err as string}>  {g('guide')}   G£ù {it.message}</Text></Box>;
        ...
        {isFullscreen ? (
          <Box flexDirection="column" width={1} marginLeft={1}>
            {Array.from({ length: trackH }).map((_, i) => (
              <Text key={i} color={i === thumbPos ? (tokens.colors.accent as string) : (tokens.colors.guide as string)}>{i === thumbPos ? 'GùÅ' : 'Göé'}</Text>
            ))}
          </Box>
        ) : null}
      </Box>
      <Box flexDirection="column">
        ...
        <Box>
          ...
          <Text wrap="wrap">{input || <Text color={tokens.colors.dim as string}>Message KlyroGÇª</Text> as unknown as string}GûÅ</Text>
        </Box>
        <Text color={tokens.colors.guide as string}>{g('rule').repeat(Math.max(10, width - 2))}</Text>
      </Box>
      <Box justifyContent="space-between">
        <Text color={tokens.colors.dim as string}>{baseHints}{maxOffset > 0 && isFullscreen ? '  -+  PgUp/Dn scroll' : ''}</Text>
        <Text color={tokens.colors.dim as string}>{cost > 0 ? `$${cost.toFixed(2)} -+ ` : ''}{ctxPct > 0 ? `${ctxPct}% ctx -+ ` : ''}{status.status === 'running' ? 'auto mode on GùÅ' : ''}</Text>
      </Box>
    </Box>
  );
}
